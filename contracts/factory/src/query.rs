...
pub fn get_vault(deps: Deps, nft_addr: &str, nft_id: &TokenId) -> StdResult<Binary> {
...
pub fn get_multi_vault(deps: Deps, nft_addr: &str) -> StdResult<Binary> {
    let nft_addr = deps.api.addr_validate(nft_addr)?;
...
pub fn get_multi_or_solo_vault(
    deps: Deps,
    nft_addr: &str,
